  ...
  const handleFileUpload = (event) => {
    ...
    reader.onload = (e) => {
      ...
      const classes = json.map((item) => ({
        starting: item.Starting || '',   // Remove the fallback to empty string
        day: item.Day || '',             // Remove the fallback to empty string
        from: item.From || '',           // Remove the fallback to empty string
        to: item.To || '',               // Remove the fallback to empty string
        room: item.Room || '',           // Remove the fallback to empty string
        location: item.Location || '',   // Remove the fallback to empty string
        class: item.Class || '',         // Remove the fallback to empty string
        description: item.Description || 'No additional details available', // Keep this fallback
      }));
